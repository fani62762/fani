...
class Viewordcomp extends StatefulWidget {
  const Viewordcomp({Key? key});

  @override
  _ViewordcompState createState() => _ViewordcompState();
}
...
class _ViewordcompState extends State<Viewordcomp> {
  ...
  Future<List<morder>> getworkordc() async {
    print(techname + "this is techname");
    final resp = await http2.get(
      Uri.parse('https://fani-service.onrender.com/ord/66/$tname'),
      headers: {'Content-Type': 'application/json'},
    );
    final parsed = jsonDecode(resp.body) as List;
    if (this.mounted) {
